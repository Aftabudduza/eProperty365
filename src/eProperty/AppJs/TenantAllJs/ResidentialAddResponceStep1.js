...
$(document).ready(function () {
    ...
    sShowOn = $("#hdnScreenType").val();

    if (sShowOn === "undefined" || sShowOn === "") {
        ...
        $("input[name=r3][value='No']").closest('div').addClass('checked');
        $("input[name=r3][value='No']").attr('checked', true);
    }
    else {
        if (sShowOn === "1") {
            $("#di").attr('disabled', true);
            ...
            $('input[name=r3]').closest('div').removeClass('checked');
            $('input[name=r3]').attr('checked', false);
            $("input[name=r3][value='Yes']").closest('div').addClass('checked');
            $("input[name=r3][value='Yes']").attr('checked', true);
        }
        else {
            $("#di").attr('disabled', true);
            $("#Dealer").attr('disabled', true);
...
function LoadAddDDL(parameters) {
    ...
    $("#numberofPeople").val("1").trigger('change');
    //.................. load State ...............//
            ...
            if (step1objAll == null || step1objAll == "") {
                applicationFree.then((data) => {
                    ...
                    if (app != null) {
                       ...
                       // $(".btnPaymentGetway").attr("total_amount", app.TotalFee);
                ...
                if (step1obj == null || step1obj == "") {
                   ...
                   // $(".btnPaymentGetway").attr("total_amount", appfee);
                }
                else {
                    submitt = true;
...
$(document).on('ifChanged', "input[type=radio][name=r3]", function (parameters) {
    debugger;
    var curVal = parseInt($("#numberofPeople").val());
    var ApplicationFee = parseInt($("#hdnApplicationFee").val());
    var ScreeningFee = parseInt($("#hdnScreeningFee").val());  
    if ($(this).attr('id') == 'di') {
        if (curVal != -1) {            
            var newApplicationFee = curVal * (ApplicationFee + ScreeningFee);
            $("#TotalApp").val(newApplicationFee);
        }
        else
        {
            var newApplicationFee = ApplicationFee + ScreeningFee;
            $("#TotalApp").val(newApplicationFee);
        }       
    }
    else if ($(this).attr('id') == 'Dealer') {
        if (curVal != -1) {
            var newApplicationFee = curVal * (ApplicationFee);
            $("#TotalApp").val(newApplicationFee);
        }
        else {
            var newApplicationFee = ApplicationFee;
            $("#TotalApp").val(newApplicationFee);
        }       
    }
    else {
...
function ValidateCash(parameters) {
    var isresult = true;
...
function GetSavedObj(creditType) {
    ...
    if (creditType == 'Credit') {
        obj = {
            ...
            "NameOfAccount": $("#nameAccountapp1Txt").val().trim(),
...
//$(document).on('change', '#TotalApp', function (parameters) {
//    
//    if ($(this).val() != '-1') {
//        var curVal = parseFloat($(this).val()).toFixed(2);
//        $(".btnPaymentGetway").attr("total_amount", curVal);
//        sTotal = curVal;
//    }

//});

//$("#TotalApp").change(function () {
//    
//    var compName = $(this).val();
//    if (compName.length >= 0) {
//        var twoPlacedFloat = parseFloat(compName).toFixed(2)
//          $(".btnPaymentGetway").attr("total_amount", twoPlacedFloat);
//        sTotal = twoPlacedFloat;
//    }
//});

//$(document).on("keyup", "#TotalApp", function (parameters) {
//    
//    var compName = $(this).val();
//    if (compName.length >= 0) {
//        var twoPlacedFloat = parseFloat(compName).toFixed(2)
