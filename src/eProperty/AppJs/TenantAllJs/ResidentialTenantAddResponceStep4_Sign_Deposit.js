...
function BindAdditionalDoc(result) {
    ...
    if (result.length > 0) {
        ...
        $.each(result, function (i, obj) {
            ...
            content += "<td><label>" + obj.DocumentDescription + "</label><input type='hidden' value='" + obj.GetWay + "' /></td>";
...
function SavedocumentUpload(parameters) {
    ...
    if (document.getElementById("documentUpload").value != "") {
        ...
        if (extFile == "jpg" || extFile == "jpeg" || extFile == "png" || extFile == 'gif' || extFile == 'tiff' || extFile == 'pdf' || extFile == 'docx' || extFile == 'doc' || extFile == 'txt') {
            ...
            notify('danger', "Only .doc, ,docx, .txt, .pdf are allowed!");
            $("#documentUpload").val("");
        }
...
function UpdateFilesDoc(evt) {
    if ($("#documentUpload").val() != "") {
        ...
        var DocId = $(G_CurRow).find('td:eq(0)').find('input[type="hidden"]').val();
        var Getway = $(G_CurRow).find('td:eq(1)').find('input[type="hidden"]').val();
        //var docId=
        ...
        $.ajax({
            type: "POST",
            url: pagePath,
            data: "{ 'Image':'" + ImageSave + "' , 'ImageName':'" + G_ImageName + "','DocId':'" + DocId + "','CurrentStatus':'Uploaded','Getway':'" + Getway + "' }",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            error:
                function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        notify('danger', "Please select file");
        $("#documentUpload").val("");
    }

}
function checkUncheckOperation(nxtRow) {
    if (nxtRow !== 'undefined' || nxtRow !== '' || nxtRow !== null) {
        $((nxtRow).find('td:eq(0)').find('input[type="checkbox"]')).prop("disabled", false);
        $((nxtRow).find('td:eq(3)').find('input[type="button"]')).prop("disabled", false);
        let status = $((nxtRow).find('td:eq(0)').find('input[type="checkbox"]')).attr('data_status');
    ...
    if (AdditionalDoc != null || AdditionalDoc > 0) {
        $("#tblDoc tbody tr").each(function (parameters) {
            ...
            if ($($(this).find('td:eq(0)').find('input[type=checkbox]')).is(':checked')) {
                ...
                notify('danger', "All the Additional documents Status must be Uploaded");
                return false;
            }
        });
    }
...
$(document).on("keyup", "#txtSecurityDeposite", function (parameters) {
    var SecurityDeposit = parseFloat($("#txtSecurityDeposite").val());
    var OneMonthRent = parseFloat($("#txtOneMonthRent").val());    
    var FirtMonthRent = parseFloat($("#txtFirtMonthRent").val());
    var TotalDue = parseFloat($("#txtTotalDue").val());

    if (SecurityDeposit === "undefined" || SecurityDeposit === "") {
        SecurityDeposit = 0;
    }
    if (OneMonthRent === "undefined" || OneMonthRent === "") {
        OneMonthRent = 0;
    }

    if (FirtMonthRent === "undefined" || FirtMonthRent === "") {
        FirtMonthRent = 0;
    }

    if (FirtMonthRent > 0) {
        TotalDue = SecurityDeposit + FirtMonthRent;
    }
    else
    {
        TotalDue = SecurityDeposit + OneMonthRent;
    }
    
    $("#txtTotalDue").val(TotalDue);
    $("#txtAmountOff").val(TotalDue);
    $("#subTotalCharge").text(TotalDue);

    var typeChck = parseInt($("#hdnFeeType").val());
    var percentRatio = parseFloat($("#hdnPercentage").val());
    var Amount = parseFloat($("#hdnFlat").val());

    var newApplicationFee = TotalDue;
    
    if (typeChck == 1) {
        $("#percentRatio").text(percentRatio.toString() + "%");
        var charge = newApplicationFee * percentRatio / 100;

        $("#Amount").text(charge.toString());
        var TotalAmount = newApplicationFee + charge;
        $("#TotalAmountCharge").text(TotalAmount);

    } else {
        $("#percentRatio").text("");
        $("#Amount").text(Amount);
        var TotalAmount = newApplicationFee + Amount;
        $("#TotalAmountCharge").text(TotalAmount);
    }

});
$(document).on("keyup", "#txtOneMonthRent", function (parameters) {
    var SecurityDeposit = parseFloat($("#txtSecurityDeposite").val());
    var OneMonthRent = parseFloat($("#txtOneMonthRent").val());
    var FirtMonthRent = parseFloat($("#txtFirtMonthRent").val());
    var TotalDue = parseFloat($("#txtTotalDue").val());
...
$(document).on("keyup", "#txtFirtMonthRent", function (parameters) {
    var SecurityDeposit = parseFloat($("#txtSecurityDeposite").val());
    var OneMonthRent = parseFloat($("#txtOneMonthRent").val());
    var FirtMonthRent = parseFloat($("#txtFirtMonthRent").val());
    var TotalDue = parseFloat($("#txtTotalDue").val());
    $("#txtProrateAmount").val(FirtMonthRent);
...
$(document).on("keyup", "#txtProrateAmount", function (parameters) {
    $("#txtFirtMonthRent").val($("#txtProrateAmount").val());
...
$(document).on("keyup", "#txtTotalDue", function (parameters) {
    ...
    var newApplicationFee = parseFloat($(this).val());

    $("#txtAmountOff").val(newApplicationFee);
    $("#subTotalCharge").text(newApplicationFee);
