...
{
    public partial class AddResidentialUnitold : Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["UserObject"] != null)
                {
                    ...
                    if (objGlobal != null)
                    {
                        ...
                        {
                            txtApplicationFee.Text = Convert.ToDecimal(objGlobal.ApplicationFee).ToString("#.00");
                            hdnApplicationFee.Value = Convert.ToDecimal(objGlobal.ApplicationFee).ToString("#.00");
                        }
                    }
                }
            }
        ...
        private ResidentialUnit SetData(ResidentialUnit obj)
        {
            ...
            {
                obj.Serial = Session["ResidentialUnitSerial"].ToString();
                obj.UpdatedBy = Convert.ToInt16(((UserProfile)Session["UserObject"]).Id);
                obj.UpdatedDate = DateTime.Now;
            }
            else
            {
                obj.Serial = new ResidentialUnitDa().MakeAutoGenLocation("1", "ResidentialUnit");
            ...
            obj.SpecialStatements =
                !string.IsNullOrEmpty(txtSpecialComments.Text) ? txtSpecialComments.Text : string.Empty;
            return obj;
        }
        ...
        protected void btnSaveBasicUnit_Click(object sender, EventArgs e)
        {
            ...
            if (_errStr.Length <= 0)
            {
                ...
                {
                    if (new ResidentialUnitDa().Insert(obj))
                    {
                        Utility.RunCMD(sql);
                        Utility.RunCMDMain(sql);
                        //Session["AddResidentialUnitId"] = null;
                        //ClearControls();
                        if (ValidateSpecs().Length <= 0)
                        {
                            ...
                            {
                                if (new ResidentialUnitDa().InsertSpecs(objResidentialUnitSpecs))
                                {
                                    Session["ResidentialUnitSpecsId"] = null;
                                }
                            }
        ...
        protected void btnSaveSpecs_Click(object sender, EventArgs e)
        {
            ...
            if (_errStr.Length <= 0)
            {
                ...
                else
                {
                    ...
                    unitObj.Serial = Session["ResidentialUnitSerial"].ToString();
                    ...
                    if (new ResidentialUnitDa().Insert(unitObj))
                    {
                        Utility.RunCMD(sql);
                        Utility.RunCMDMain(sql);
                    }
                ...
                {
                    if (new ResidentialUnitDa().InsertSpecs(obj))
                    {
                        Session["ResidentialUnitSpecsId"] = null;
        ...
        private ResidentialUnitSpecs SetDataForSpecs(ResidentialUnitSpecs obj)
        {
            ...
            obj.UnitSize = Convert.ToDecimal(txtTotalSize.Text) >= 0 ? Convert.ToDecimal(txtTotalSize.Text) : 0;
            obj.UnitNumberFloors =
                Convert.ToInt32(txtNumberfloors.Text) > 0 ? Convert.ToInt32(txtNumberfloors.Text) : 0;
            obj.UnitMonthlyRent = Convert.ToDecimal(txtMonthlyRentAmount.Text) > 0
                ? Convert.ToDecimal(txtMonthlyRentAmount.Text)
                : 0;
            obj.UnitLotSize = Convert.ToInt32(txtMonthlyRentAmount.Text) > 0
                ? Convert.ToInt32(txtMonthlyRentAmount.Text)
                : 0;
            obj.UnitApplicationFee = Convert.ToDecimal(txtApplicationFee.Text) > 0
                ? Convert.ToDecimal(txtApplicationFee.Text)
                : 0;
            obj.UnitAgentName = !string.IsNullOrEmpty(txtAgentName.Text) ? txtAgentName.Text : string.Empty;
            obj.UnitAgentPhone = !string.IsNullOrEmpty(txtAgentPhoneNumber.Text)
                ? txtAgentPhoneNumber.Text
                : string.Empty;
            obj.UnitBrokerName = !string.IsNullOrEmpty(txtBroker.Text) ? txtBroker.Text : string.Empty;
            obj.UnitBrokerPhone = !string.IsNullOrEmpty(txtBrokerPhoneNumber.Text)
                ? txtBrokerPhoneNumber.Text
                : string.Empty;

            obj.UnitNumberOfBaths = Convert.ToInt32(ddlNumberofBaths.SelectedValue) > 0
                ? Convert.ToInt32(ddlNumberofBaths.SelectedValue)
                : -1;
            obj.UnitNumberBedrooms = Convert.ToDecimal(ddlNumberBedrooms.SelectedValue) > 0
                ? Convert.ToInt32(ddlNumberBedrooms.SelectedValue)
                : -1;
            ...
            obj.UnitSpecialStatements = !string.IsNullOrEmpty(txtSpecialStatementBtm.Text)
                ? txtSpecialStatementBtm.Text
                : string.Empty;
            return obj;
        }
        ...
        public static string SaveCommunication(ResidentialCommunication obj) 
        {
            try
            {
                try
                {
                    ...
                    mail.Body = obj.Massage.ToString();
